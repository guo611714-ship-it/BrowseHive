...
完成所有 skill 评审并生成 verdicts 后，**不要自己修改代码**。调用 `submit_fix_manifest` 工具，由 ParallelFixEngine 并行执行。

#### 4.1 构建修复清单

从 Phase 2 的 results.json 中提取 actionable verdicts（Retire/Improve/Merge），调用工具：
...
{
  "success": true,
  "summary": "source=stocktake: 12 tasks executed",
  "task_count": 12,
  "conflicts": [],
  "details": [...]
}
...
如有失败任务，提供两个选项：
1. **重试**：重新调用 `submit_fix_manifest`，仅包含失败的任务
2. **手动处理**：用户自行修复
