...
## Parallel Fix (after review)

当 structured review 返回 accepted/actionable findings 时，**不要手动逐个修复**。
调用 `submit_fix_manifest` 工具，由 ParallelFixEngine 并行执行所有修复：
...
- `strategy: "file"` — 同文件串行，不同文件并行（避免 review findings 之间的冲突）
- 工具返回后，检查 `conflicts` 列表，如有冲突则手动处理
- 修复完成后，**必须重新运行 structured review** 验证无遗留 finding
- 循环直到 review clean（无 accepted findings）
