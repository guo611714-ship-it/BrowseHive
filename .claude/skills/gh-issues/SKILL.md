...
如果 issues 数量 ≥ 3 且涉及不同文件，可以先调用 `submit_fix_manifest` 并行预修复：
...
引擎会并行修改各 issue 涉及的文件。之后 worker 只需处理 git 操作（branch/commit/push/PR）。
...
- notification route.
- **如果 Phase 5.1 已预修复**，包含修复后的文件路径列表。
...
如果 Phase 5.1 已预修复:
  只需 git add <已修改文件> 并跳到 Commit
否则:
  Implement minimal fix（可在 fix 过程中调用 fix_manifest 工具）
