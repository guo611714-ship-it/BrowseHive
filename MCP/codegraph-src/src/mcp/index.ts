...
export class MCPServer {
  ...
  async start(): Promise<void> {
    // HTTP transport mode — takes priority over everything
    if (process.env.CODEGRAPH_TRANSPORT === 'http') {
      return this.startDirect('HTTP transport mode');
    }
  ...
  private async startDirect(reason: string): Promise<void> {
    ...
    // HTTP transport mode
    if (process.env.CODEGRAPH_TRANSPORT === 'http') {
      const port = parseInt(process.env.CODEGRAPH_HTTP_PORT || '8091', 10);
      await this.startHttpMode(port);
      return;
    }
  ...
  /** HTTP Streamable transport mode — multi-session via HTTP. */
  private async startHttpMode(port: number): Promise<void> {
    ...
    // Stateless mode: each request creates a fresh transport + session
    app.post('/mcp', async (req: any, res: any) => {
      try {
        ...
        // Wrap StreamableHTTPServerTransport as JsonRpcTransport
        const handler = (await import('./http-adapter')).createHttpAdapter(mcpTransport);
        const session = new MCPSession(handler, this.engine!, {
          explicitProjectPath: this.projectPath,
        });
        if (this.projectPath) {
          void this.engine!.ensureInitialized(this.projectPath);
        ...
        process.stderr.write(`[CodeGraph MCP] HTTP error: ${err}\n`);
        if (!res.headersSent) {
    ...
    app.listen(port, '127.0.0.1', () => {
      process.stderr.write(`[CodeGraph MCP] HTTP mode on http://127.0.0.1:${port}/mcp\n`);
    });
